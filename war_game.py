...
def higher_value(player_draw, computer_draw, computer, player):
    ...
    if card_value(player_draw) < card_value(computer_draw):
        """
        If the player < computer, use a for loop to add each card in the player list to the computer list.
        Then give that list to the computer object won_turn method.
        """
        ...
        """
        If the player > computer, use a for loop to add each card in the computer list to the player list.
        Then give that list to the player object won_turn method.
        """
    ...
    def turns():
        ...
        while len(player.cards) != 0 and len(computer.cards) != 0:
            ...
            if card_value(player_draw) == card_value(computer_draw):
                ...
                for x in [player.play_card(4)]:     # loop to add each item in the list returned from the play_card
                                                    # object method to the list of the cards being played in the turn.
                    player_draw += x
